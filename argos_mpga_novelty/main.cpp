 ...
 * experiment, the score is the novelty of the genome. The aggregate
 * is the average novelty over several trials.
 */
Real ScoreAggregator(const std::vector<Real>& vec_scores) {
   Real fScore = vec_scores[0];
   for(size_t i = 1; i < vec_scores.size(); ++i) {
      fScore = fScore + vec_scores[i];
   }
   return fScore/vec_scores.size();
}
